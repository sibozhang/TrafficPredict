...
class DataLoader():
    ...
    def __init__(self, batch_size=50, seq_length=12, datasets=[0, 1, 2],choose_test=False, forcePreProcess=False, infer=False):
        ...
        data_file = os.path.join(self.data_dir, "dense_type2_gap4_normal.cpkl")
           ...
           # print('frameList size:',numFrames)
            ...
            for ind, frame in enumerate(frameList):
                ...
                for ped in range(len(pedsInFrame)):
                    ...
                    pedsWithPos.append([current_id, (current_type)/5.0, (current_position[0]-min_x)/gap_x, (current_position[1]-min_y)/gap_y,
                                        current_position[2],current_velocity[0], current_velocity[1], current_velocity[2], current_width,current_length, current_heading])
    ...
    def load_test_preprocessed(self, data_file):
        ...
        self.test_data = len(self.data)*5/6
        ...
        print('seq, counter, dataset: {}:{}:{}'.format(self.seq_length,counter,len(self.data)/4))
    ...
    def load_preprocessed(self, data_file):
        ...
        # Calculate the number of batches
        self.num_batches = int(counter/self.batch_size)
        self.valid_num_batches = self.num_batches/4
        self.test_data = len(self.data)*5/6
        #pdb.set_trace()
    ...
    def tick_batch_pointer(self, valid=False):
        ...
        if not valid:
            ...
            if self.choose_test:
                ...
                if self.dataset_pointer >= self.test_data:
                    self.dataset_pointer = 0
        else:
            # Go to the next dataset
            ...
            # If all datasets are done, then go to the first one again
            if self.valid_dataset_pointer >= len(self.data):
                self.valid_dataset_pointer = self.test_data+1
