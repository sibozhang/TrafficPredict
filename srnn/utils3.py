...
class DataLoader():
    ...
    def __init__(self, batch_size=50, seq_length=12, datasets=[0, 1, 2],choose_test=False, forcePreProcess=False, infer=False):
        ...
        data_file = os.path.join(self.data_dir, "new_data_gap4.cpkl")
        #pdb.set_trace()
    ...
    def load_preprocessed(self, data_file):
        ...
        self.frameList = self.raw_data[1]
        self.numPedsList = self.raw_data[2]
        #self.data = self.data[0:len(self.data)*5/6]
    ...
    def point_rotate(self, x, y, angle):
        anglepi = angle * math.pi / 180.0
        X = math.cos(anglepi) * x + math.sin(anglepi) * y
        Y = -math.sin(anglepi) * x + math.cos(anglepi) * y
        return X, Y
    ...
    def next_batch(self, randomUpdate=True):
        ...
        while i < self.batch_size:
            ...
            if idx + self.seq_length < len(frame_data):
                ...
                if randomUpdate:
                    self.frame_pointer += random.randint(5, self.seq_length)
                    #self.frame_pointer += 9
                    #angle = random.randint(-60, 60)
                    #if angle % 2 == 0:
                    #    for idx in range(len(seq_source_frame_data)):
                    #        for idx2 in range(len(seq_source_frame_data[idx])):
                    #            x1 = seq_source_frame_data[idx][idx2]
                    #            x_tmp = x1[2]
                    #            y_tmp = x1[3]
                    #            x_tmp2, y_tmp2 = self.point_rotate(x_tmp,y_tmp, angle)
                    #            seq_source_frame_data[idx][idx2][2] = x_tmp2
                    #            seq_source_frame_data[idx][idx2][3] = y_tmp2
                else:
