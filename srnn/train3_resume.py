...
def trainFromEpoch(net, optimizer, epoch_path):
    #pdb.set_trace()
...
def train(args):
    ...
    start_epoch = 0
    if args.resume == 1:
        net, optimizer = trainFromEpoch(net, optimizer, checkpoint_path(args.epoch))
        start_epoch = args.epoch
    else:
        print("start from scartch")
        init_weights(net, lstm='kaiming')
    #optimizer = torch.optim.Adam(net.parameters(), lr=args.learning_rate, weight_decay=1e-4)
