...
class DataLoader():
    ...
    def __init__(self, batch_size=50, seq_length=12, datasets=[0, 1, 2],choose_test=False, forcePreProcess=False, infer=False):
        ...
        data_file = os.path.join(self.data_dir, "type2_gap4_normal_2_new.cpkl")
        #pdb.set_trace()
    ...
    def load_test_preprocessed(self, data_file):
        ...
        self.test_data = len(self.data)*4/5
    ...
    def extendData(self):
        ...
        new_dataset = []
        origin_data_length = len(self.data)
        for dataset in range(origin_data_length):
            newD = self.addRotatedDataset(self.data[dataset])
            new_dataset.append(np.array(newD))

        for dataset in range(origin_data_length):
            new_dataset.append(self.data[dataset])
        ...
        for index in range(len(self.raw_data[1])):
            self.frameList.append(self.raw_data[1][index])
        for index in range(len(self.raw_data[2])):
            self.numPedsList.append(self.raw_data[2][index])
        for index in range(len(self.raw_data[3])):
            self.valid_data.append(self.raw_data[3][index])

        self.data = new_dataset

    def addRotatedDataset(self,dateset):

        rotate_angle = random.uniform(-0.4, -2)

        x_index = 2
        y_index = 3

        cos_value = np.cos(rotate_angle)
        sin_value = np.sin(rotate_angle)
        ...
        for frameid in range(len(dateset)):
            framedata = dateset[frameid]

            new_agents = []
            for pedid in range(len(framedata)):
                data = framedata[pedid]

                temp_x = data[x_index]
                temp_y = data[y_index]
                data[x_index] = temp_x*cos_value + temp_y*sin_value
                data[y_index] = -temp_x*sin_value + temp_y*cos_value
            ...
            new_frames.append(np.array(new_agents))
        ...
        for frameid in range(len(new_frames)):
            for pedid in range(len(new_frames[frameid])):
                new_frames[frameid][pedid][x_index] = ((new_frames[frameid][pedid][x_index] - min_x)/maxOne -0.5)*2
                new_frames[frameid][pedid][y_index] = ((new_frames[frameid][pedid][y_index] - max_y)/maxOne -0.5)*2
    ...
    def load_preprocessed(self, data_file):
        ...
        self.frameList = self.raw_data[1]
        self.numPedsList = self.raw_data[2]
        self.data = self.data[0:len(self.data)*4/5]

        #extend dataset by rotating
        self.extendData()
