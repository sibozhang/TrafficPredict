...
def main():
    ...
    # Model to be loaded
    parser.add_argument('--epoch', type=int, default=102,
    ...
    # Save directory
    # save_directory = 'save/'
    save_directory = 'save/' + str(sample_args.flag) + '/'
    save_directory += str(sample_args.test_dataset)+'/'
    save_directory += 'save_attention'
    use_model_directory = 'save/'+str(sample_args.flag)+'/5/save_attention'
    # Define the path for the config file for saved args
    with open(os.path.join(use_model_directory, 'config.pkl'), 'rb') as f:
    ...
    net = net.cuda()
    net.eval()
    checkpoint_path = os.path.join(use_model_directory, 'srnn_model_'+str(sample_args.epoch)+'.tar')
    #pdb.set_trace()
    ...
    dataloader = DataLoader(1, sample_args.pred_length + sample_args.obs_length, dataset, True, False, infer=True)
    ...
    for batch in range(dataloader.num_batches):
        ...
        obs_nodes, obs_edges, obs_nodesPresent, obs_edgesPresent = nodes[:sample_args.obs_length], edges[:sample_args.obs_length], \
                                                                   nodesPresent[:sample_args.obs_length], edgesPresent[:sample_args.obs_length]

        # Sample function
        ret_nodes, ret_attn = sample(obs_nodes, obs_edges, obs_nodesPresent, obs_edgesPresent, sample_args, net, nodes, edges, nodesPresent)
        #pdb.set_trace()
        #if batch < 10:
        #    print(ret_nodes.data.cpu().numpy())
...
def get_min_max(next_x):
    next_x = max(-1, next_x)
    next_x = min(1, next_x)

    return next_x

def min_max_flag(next_x):
    if next_x < -1:
        return 0
    if next_x > 1:
        return 1
...
def sample(nodes, edges, nodesPresent, edgesPresent, args, net, true_nodes, true_edges, true_nodesPresent):
    ...
    for tstep in range(args.obs_length-1):
        # Forward prop
        # print('---------------------------------------{}'.format(tstep))
        out_obs, h_nodes, h_edges, c_nodes, c_edges, _ = net(nodes[tstep].view(1, numNodes, lanum), edges[tstep].view(1, numNodes*numNodes, lanum),
                                                             [nodesPresent[tstep]], [edgesPresent[tstep]], h_nodes, h_edges, c_nodes, c_edges)
    ...
    ret_nodes = Variable(torch.zeros(args.obs_length + args.pred_length, numNodes, lanum), volatile=True).cuda()
    ret_nodes[:args.obs_length, :, :] = nodes.clone()

    ret_edges = Variable(torch.zeros((args.obs_length + args.pred_length), numNodes * numNodes, lanum), volatile=True).cuda()
    ...
    for tstep in range(args.obs_length-1, args.pred_length + args.obs_length-1):
        ...
        # Forward prop
        outputs, h_nodes, h_edges, c_nodes, c_edges, attn_w = net(ret_nodes[tstep].view(1, numNodes, lanum), ret_edges[tstep].view(1, numNodes*numNodes, lanum),
                                                                  [nodesPresent[args.obs_length-1]], [edgesPresent[args.obs_length-1]], h_nodes, h_edges, c_nodes, c_edges)
        loss_pred = Gaussian2DLikelihoodInference(outputs, true_nodes[tstep + 1].view(1, numNodes, lanum), nodesPresent[args.obs_length-1], [true_nodesPresent[tstep + 1]])
        ...
        # next_x = get_min_max(next_x)
        # next_y = get_min_max(next_y)
        #print("next_x:,  next_y: ",(next_x, next_y))
        #import pdb
        #pdb.set_trace()
        #for idxnet in range(next_x.size(0)):
            #if min_max_flag(next_x[idxnet]) < 2:
                #print("outliner!!!!")
                #print next_x[idxnet]

                #print ret_nodes[tstep,:,0][idxnet].data.cpu().numpy()[0]
               # next_x[idxnet] = ret_nodes[tstep, :, 0][idxnet].data.cpu().numpy()[0].item()
               # next_y[idxnet] = ret_nodes[tstep, :, 1][idxnet].data.cpu().numpy()[0].item()
            #if min_max_flag(next_y[0]) < 2:
            #if min_max_flag(next_y[idxnet]) < 2:
                #print("outliner!!!!")
                ...
                #next_x[idxnet] = ret_nodes[tstep, :, 0][idxnet].data.cpu().numpy()[0].item()
                #next_y[idxnet] = ret_nodes[tstep, :, 1][idxnet].data.cpu().numpy()[0].item()
            #    next_y = ret_nodes[tstep, :, 1].clone()

        ret_nodes[tstep + 1, :, 0] = next_x
        ret_nodes[tstep + 1, :, 1] = next_y
        temp_type = ret_nodes[tstep, :, 2].clone()
        ret_nodes[tstep + 1, :, 2] = temp_type
